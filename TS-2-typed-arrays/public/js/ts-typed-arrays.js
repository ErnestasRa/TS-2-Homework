...
console.group('Užduotys');
{
    ...
    {
        const firstWordLetters = (letter, word) => letter + word[0];
        const createWord = (words) => {
            const word = words.reduce(firstWordLetters, '');
            return word;
        };
        const words = [
            ['Lama', 'Ezeras', 'Gandras', 'Oras'],
            ['Oda', 'Hello', 'Ona'],
            ['Lempa', 'Udra', 'Lemon'],
        ];
        words.forEach((words) => console.log(`[${words.join(', ')}] ========= ${createWord(words)}`));
    }
    console.groupEnd();
    console.group('3. Sukurkite ir tipais aprašykite funkciją, kuri saudaugintų visus number masyvo skaičius');
    {
        ...
        numbersArray.forEach(numberArray => {
            numberArray.map(number => {
                const multiplied = multiply(numberArray.length, number);
...
console.groupEnd();
//# sourceMappingURL=ts-typed-arrays.js.map